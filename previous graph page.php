<!DOCTYPE html>
<html>
<head>

 <title>Chart Library</title>
 <link rel="stylesheet" href="css/bootstrap.min.css">
 <link rel="stylesheet" href="css/bootstrap-theme.min.css">
 <link rel="stylesheet" href="css/style.css">

</head>
<body>
<header id ="titlelogo2">
  <div class="container">
    <h1> Chart Library </h1>
  </div>
</header>


  <div id ="Homebutton">
    <div class= "container">


      <div class="row">

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="images/pies.png" alt="...">
            <div class="caption">
              <h3>Chart 1</h3>
              <p>Year 11's results</p>
              <p><a href="Graphs/year11_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="images/bars.png" alt="...">
            <div class="caption">
              <h3>Chart 2</h3>
              <p>Exam 1 results</p>
              <p><a href="Graphs/exam1_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 3</h3>
              <p>Year 9's results</p>
              <p><a href="Graphs/year9_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

      </div>
      <div class="row">

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 4</h3>
              <p>Number of students that passed</p>
              <p><a href="Graphs/final_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 5</h3>
              <p>Year 8s Results</p>
              <p><a href="Graphs/year8_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 6</h3>
              <p>Exam 2 Results</p>
              <p><a href="Graphs/exam2_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

      </div>
      <div class="row">

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 7</h3>
              <p>Number Cheating Attempted by Students</p>
              <p><a href="Graphs/cheatingstudents.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 8</h3>
              <p>Number of Sick Students</p>
              <p><a href="Graphs/sickstudents.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

        <div class="col-sm-12 col-md-4">
          <div class="thumbnail">
            <img src="..." alt="...">
            <div class="caption">
              <h3>Chart 9</h3>
              <p>Exam 3 Results</p>
              <p><a href="Graphs/exam3_results.php" class="btn btn-primary" role="button">View</a> </p>
            </div>
          </div>
        </div>

      </div>

    </div>
  </div>

</body>
</html>
